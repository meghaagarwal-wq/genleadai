...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# Iter77 — Weekly Recap PDF export (Section 7)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
from fastapi import Response  # noqa: E402


def _delta_chip(d: int) -> str:
    if d > 0:
        return f"▲ +{d}%"
    if d < 0:
        return f"▼ {d}%"
...
@router.get("/weekly-recap/export.pdf")
async def weekly_recap_pdf(current_user: dict = Depends(get_current_user)):
    """Render the same weekly_recap payload as a polished, downloadable PDF."""
    recap = await weekly_recap(current_user)
    workspace_name = current_user.get("tenant_name") or current_user.get("active_tenant_name") or "Your workspace"
    ...
    c.drawString(2 * cm, height - 2 * cm, "Aria Weekly Recap")
    c.setFont("Helvetica", 11)
    c.drawString(2 * cm, height - 2.7 * cm, workspace_name)
    c.setFont("Helvetica-Oblique", 9)
    c.drawRightString(width - 2 * cm, height - 2.7 * cm, recap.get("headline", ""))

    # ── Narrative ──
    y = height - 5.2 * cm
    c.setFillColor(HexColor("#1A0A2E"))
    c.setFont("Helvetica-Bold", 11)
    c.drawString(2 * cm, y, "Aria's read on the week")
    y -= 0.55 * cm
    c.setFont("Helvetica", 10)
    narrative = recap.get("narrative") or ""
    # naive wrap
    ...
    # ── 6 KPI tiles in 2 rows × 3 cols ──
    y -= 0.4 * cm
    tile_w = (width - 4 * cm - 2 * 0.4 * cm) / 3
    tile_h = 2.4 * cm
    stats = recap.get("stats", [])[:6]
    for i, s in enumerate(stats):
        col = i % 3
        row = i // 3
        x = 2 * cm + col * (tile_w + 0.4 * cm)
        ty = y - row * (tile_h + 0.4 * cm)
        c.setFillColor(HexColor("#F9F5FF"))
        c.roundRect(x, ty - tile_h, tile_w, tile_h, 10, fill=1, stroke=0)
        c.setFillColor(HexColor("#5A4A7A"))
        c.setFont("Helvetica-Bold", 8)
        c.drawString(x + 0.4 * cm, ty - 0.6 * cm, s["label"].upper())
        c.setFillColor(HexColor("#1A0A2E"))
        c.setFont("Helvetica-Bold", 20)
        c.drawString(x + 0.4 * cm, ty - 1.4 * cm, str(s["value"]))
        delta_color = "#10B981" if s["delta"] > 0 else "#EF4444" if s["delta"] < 0 else "#9B8AB0"
        c.setFillColor(HexColor(delta_color))
        c.setFont("Helvetica-Bold", 9)
        c.drawString(x + 0.4 * cm, ty - 2.0 * cm, _delta_chip(s["delta"]))

    y = y - 2 * (tile_h + 0.4 * cm) - 0.6 * cm

    # ── Highlights box ──
    def _box(title, body, ycur):
        c.setFillColor(HexColor("#E8E0F5"))
        c.roundRect(2 * cm, ycur - 1.6 * cm, width - 4 * cm, 1.6 * cm, 8, fill=1, stroke=0)
        c.setFillColor(HexColor("#1A0A2E"))
        c.setFont("Helvetica-Bold", 10)
        c.drawString(2.4 * cm, ycur - 0.6 * cm, title)
        c.setFont("Helvetica", 10)
        c.drawString(2.4 * cm, ycur - 1.2 * cm, (body or "—")[:110])
        return ycur - 1.9 * cm

    y = _box("BIGGEST WIN", recap.get("biggest_win") or "No standout win this week.", y)
    y = _box("BIGGEST MISS", recap.get("biggest_miss") or "Nothing dropped through the cracks.", y)
    y = _box("TOP CHANNEL", recap.get("top_channel") or "—", y)

    # ── Focus plan ──
    y -= 0.2 * cm
    c.setFillColor(HexColor("#1A0A2E"))
    c.setFont("Helvetica-Bold", 11)
    c.drawString(2 * cm, y, "Next week's focus")
    y -= 0.5 * cm
    c.setFont("Helvetica", 10)
    for i, f in enumerate(recap.get("next_week_focus", [])[:3], start=1):
        c.setFillColor(HexColor("#7C35DC"))
        c.drawString(2 * cm, y, f"{i}.")
        c.setFillColor(HexColor("#1A0A2E"))
        c.drawString(2.6 * cm, y, (f or "")[:100])
        y -= 0.5 * cm

    # ── Footer ──
    c.setFillColor(HexColor("#9B8AB0"))
    c.setFont("Helvetica-Oblique", 8)
    c.drawString(2 * cm, 1.5 * cm, f"Generated by Aria · GenLeadAI · {now_iso_date()}")
    ...
    filename = f"Aria-Weekly-Recap-{datetime.now(timezone.utc).strftime('%Y-%m-%d')}.pdf"
    return Response(
        content=pdf_bytes,
