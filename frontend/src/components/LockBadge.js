...
const LockBadge = ({ featureKey, label = 'Upgrade', size = 'sm' }) => {
  ...
  return (
    <span
      ...
      onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.stopPropagation(); openUpgrade(featureKey); } }}
      className={`inline-flex items-center gap-1 font-bold uppercase tracking-wider rounded border bg-[#F4F0FF] text-[#7C35DC] border-[#7C35DC]/30 hover:bg-[#E0D4F7] transition-colors cursor-pointer select-none ${sizeClasses}`}
