...
const Tile = ({ icon: Icon, label, value, accent = '#0F766E', testid }) => (
  ...
  return (
    <div data-testid="pt-overview-page">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 mb-6">
        ...
        <Tile testid="metric-pos-replies"     icon={ChatCircleDots}   label="Positive replies" value={m.positive_replies}        accent="#0F766E" />
      ...
      {data?.is_empty && (
        <div className="bg-white border border-dashed border-[#CBD5E1] rounded-xl p-10 text-center" data-testid="pt-overview-empty">
          ...
          <div className="mt-4 flex items-center justify-center gap-2">
            <Link to="/pt/leads" className="inline-flex items-center gap-1.5 px-3 py-2 rounded-md text-sm font-semibold text-white" style={{ background: '#0F766E' }} data-testid="overview-cta-upload">
              Upload CSV <ArrowRight size={14} weight="bold" />
            </Link>
            <Link to="/pt/integrations" className="inline-flex items-center gap-1.5 px-3 py-2 rounded-md text-sm font-semibold text-[#0F766E] border border-[#0F766E]/30" data-testid="overview-cta-connect">
...
const ConnBanner = ({ connected, platform, msg }) => (
  <div className="flex items-center gap-3 p-3 rounded-lg border" style={{ borderColor: connected ? '#0F766E33' : '#CBD5E1', background: connected ? '#F0FDFA' : '#FFFFFF' }}>
    <Plugs size={18} weight="duotone" className={connected ? 'text-[#0F766E]' : 'text-[#94A3B8]'} />
    <div className="flex-1">
      <div className="text-xs font-bold uppercase tracking-[0.16em]" style={{ color: connected ? '#0F766E' : '#475569' }}>{platform}</div>
      <div className="text-sm text-[#0F172A]">{msg}</div>
    </div>
    {!connected && (
      <Link to="/pt/integrations" className="text-xs font-semibold text-[#0F766E] hover:underline">Connect</Link>
    )}
  </div>
);
